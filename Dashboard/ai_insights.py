...
# =============================================================================
# OPERATIONAL TARGETS — the goalposts a utility manager pins on the wall.
# Used by the Daily Briefing and by chart overlays to show actuals vs target.
# =============================================================================

TARGETS = {
    "nrw":                  {"value": 25.0, "unit": "%",       "direction": "lower_is_better",
                              "label": "Non-revenue water",      "icon": "water_drop"},
    "collection_efficiency":{"value": 90.0, "unit": "%",       "direction": "higher_is_better",
                              "label": "Collection efficiency",  "icon": "payments"},
    "service_hours":        {"value": 18.0, "unit": "hrs/day", "direction": "higher_is_better",
                              "label": "Service continuity",     "icon": "schedule"},
    "coverage":             {"value": 80.0, "unit": "%",       "direction": "higher_is_better",
                              "label": "Service coverage",       "icon": "public"},
}
...
class InsightsEngine:
    ...
    def detect_anomalies(self, lookback_days: int = 7) -> List[Dict]:
        ...
        # Augment with statistical anomaly detection
        try:
            from analytics.anomaly_detection import run_full_detection
            svc_df = pd.DataFrame()  # service data not always available here
            statistical_anomalies = run_full_detection(self.billing_df, self.prod_df, svc_df)
            for sa in statistical_anomalies:
                # Avoid duplicates by checking metric name
                if not any(a["metric"] == sa.metric for a in anomalies):
                    anomalies.append({
                        "metric": sa.metric,
                        "change_pct": 0,
                        "severity": sa.severity,
                        "message": sa.explanation,
                        "method": sa.method,
                    })
        except Exception:
            logger.debug("Statistical anomaly detection not available", exc_info=True)
...
def generate_monthly_report(
    ...
    prod_df: pd.DataFrame,
    fin_df: pd.DataFrame,
    selected_period: str = "Current Period",
    prev_billing_df: Optional[pd.DataFrame] = None,
    prev_prod_df: Optional[pd.DataFrame] = None,
) -> str:
    """
    Generate a comprehensive monthly report with KPIs, month-over-month deltas,
    anomaly summary, and strategic recommendations.

    If previous-period DataFrames are provided, month-over-month deltas are included.
    """
    # Current period metrics
    total_billed = billing_df["billed"].sum() if not billing_df.empty else 0
    total_paid = billing_df["paid"].sum() if not billing_df.empty else 0
    coll_eff = (total_paid / total_billed * 100) if total_billed > 0 else 0
    total_prod = prod_df["production_m3"].sum() if not prod_df.empty else 0
    total_cons = billing_df["consumption_m3"].sum() if not billing_df.empty else 0
    nrw = ((total_prod - total_cons) / total_prod * 100) if total_prod > 0 else 0
    total_opex = fin_df["opex"].sum() if not fin_df.empty else 0
    total_sewer_rev = fin_df["sewer_revenue"].sum() if not fin_df.empty else 0
    total_revenue = total_paid + total_sewer_rev
    cost_recovery = (total_revenue / total_opex * 100) if total_opex > 0 else 0
    avg_svc_hours = prod_df["service_hours"].mean() if not prod_df.empty else 0
    n_customers = billing_df["customer_id"].nunique() if "customer_id" in billing_df.columns else 0

    # Deltas
    def _delta(current: float, previous_df: Optional[pd.DataFrame], col: str, agg: str = "sum") -> str:
        if previous_df is None or previous_df.empty or col not in previous_df.columns:
            return ""
        prev_val = getattr(previous_df[col], agg)() if agg == "sum" else previous_df[col].mean()
        if prev_val == 0:
            return ""
        change = ((current - prev_val) / prev_val) * 100
        arrow = "+" if change >= 0 else ""
        return f" ({arrow}{change:.1f}% MoM)"

    report = f"""# Monthly Performance Report — {selected_period}

## Key Performance Indicators

| Metric | Value | Target | Status |
|--------|-------|--------|--------|
| Collection Efficiency | {coll_eff:.1f}% | 95% | {'On Track' if coll_eff >= 90 else 'Below Target'} |
| Non-Revenue Water | {nrw:.1f}% | <25% | {'On Track' if nrw <= 25 else 'Above Target'} |
| Cost Recovery | {cost_recovery:.1f}% | >100% | {'On Track' if cost_recovery >= 100 else 'Below Target'} |
| Service Continuity | {avg_svc_hours:.1f} hrs/day | 20 hrs | {'On Track' if avg_svc_hours >= 18 else 'Below Target'} |
| Active Customers | {n_customers:,} | — | — |

## Financial Performance

- **Total Revenue**: {total_revenue:,.0f}{_delta(total_revenue, prev_billing_df, 'paid')}
- **Operating Expenditure**: {total_opex:,.0f}
- **Collection Efficiency**: {coll_eff:.1f}%{_delta(coll_eff, None, '', '')}
- **Water Revenue**: {total_paid:,.0f}
- **Sewer Revenue**: {total_sewer_rev:,.0f}

## Operational Performance

- **Water Produced**: {total_prod:,.0f} m³
- **Water Billed**: {total_cons:,.0f} m³
- **Non-Revenue Water**: {nrw:.1f}% ({(total_prod - total_cons):,.0f} m³ lost)
- **Avg Service Hours**: {avg_svc_hours:.1f} hrs/day
...
"""
    ...
    try:
        ...
        if anomalies:
            ...
            for a in anomalies[:5]:
                ...
                report += f"- {icon} **{a['metric']}**: {a['message']}\n"
            report += "\n"
    except Exception:
        pass
    ...
    try:
        zones = engine.zone_performance_summary()
        if zones:
            report += "## Zone Performance Summary\n\n"
            report += "| Zone | Collection Eff. | NRW | Service Hrs |\n"
            report += "|------|----------------|-----|-------------|\n"
            for zone, m in sorted(zones.items(), key=lambda x: x[1].get("collection_efficiency", 0)):
                report += f"| {zone} | {m.get('collection_efficiency', 0):.1f}% | {m.get('nrw', 0):.1f}% | {m.get('service_hours', 0):.1f}h |\n"
            report += "\n"
    except Exception:
        pass
    ...
    recs = []
    if nrw > 35:
        recs.append("Launch targeted NRW reduction programme — focus on pressure management and active leak detection")
    if coll_eff < 85:
        recs.append("Strengthen revenue collection through improved billing accuracy and customer follow-up")
    if cost_recovery < 100:
        recs.append("Review cost structure to close the operating cost coverage gap")
    if avg_svc_hours < 16:
        recs.append("Investigate service interruption causes and prioritise infrastructure maintenance")
    if not recs:
        recs.append("Continue monitoring KPIs and maintain current operational standards")
