...
def scene_production():
    ...
    render_kpi_row([
        KPI("Total production", val_display,
            ...
            delta_kind="neutral",
            icon="water_drop",
            help="Total water produced across sources in the selected period.",
        ...
        KPI("Active sources", f"{active_sources} / {total_sources_count}",
            ...
            delta_kind="neutral",
            icon="hub",
            help="Sources reporting production vs total known sources.",
            sparkline=src_spark),
        KPI("Capacity utilisation", f"{cap_utilization:.1f}%",
            delta="of design capacity (est.)",
            delta_kind=util_kind,
            icon="bolt",
            help="Production ÷ estimated design capacity × 100.",
