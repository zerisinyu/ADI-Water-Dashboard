...
from ai_insights import InsightsEngine, generate_board_brief_text, generate_board_brief_llm, generate_todo_list_llm
...
def scene_executive():
    ...
    # Each briefing card links straight to its pillar page (same tab).
    _slot_href = {
        "access":  "/Access_&_Coverage",
    ...
    # Thin separator + breathing room between the briefing cards and the signals.
    st.markdown("<hr class='soft-divider'>", unsafe_allow_html=True)

    # Three equal columns: Top wins · Top risks · MajiBot's to-do. The to-do can
    # be regenerated by the configured LLM (BYOK). Wrapped in a fragment so the
    # Generate button reruns only this block, not the whole dashboard.
    @st.fragment
    def _signals_block():
        wins_col, risks_col, todo_col = st.columns([1, 1, 1])
        with wins_col:
            render_risk_card("Top wins", win_items[:4], tone="good")
        with risks_col:
            render_risk_card("Top risks", risk_items[:4], tone="warn")
        with todo_col:
            # An AI-drafted list (when generated) takes precedence over the
            # rule-based one; otherwise the threshold-driven default shows.
            # The Generate button lives inside the same bordered card (via a
            # keyed container) so it reads as part of the to-do, not a
            # separate control floating below it.
            with st.container(key="majibot-todo-card"):
                ai_todo = st.session_state.get("majibot_ai_todo")
                render_majibot_todo("MajiBot's to-do", ai_todo or todo_items[:5], bare=True)
                if is_llm_configured():
                    if st.button("Generate with AI", key="majibot_generate",
                                 help="Uses your configured API key to draft next actions"):
                        try:
                            with st.spinner("MajiBot is drafting your to-do list…"):
                                gen = generate_todo_list_llm(
                                    nrw, coll_eff, service_hours, anomalies, selected_country
                                )
                            if gen:
                                st.session_state["majibot_ai_todo"] = gen
                                st.rerun()
                            else:
                                st.caption("No suggestions returned — showing the standard checklist.")
                        except Exception:
                            st.caption("Couldn't reach the AI provider — showing the standard checklist.")
                else:
                    st.caption("Bring your own API key on the Settings page to generate AI suggestions.")
    ...
    if _is_master:
        _bench_zone_mode = bool(selected_country and selected_country != "All")
        if _bench_zone_mode:
            render_section_header(
                f"{selected_country} — cross-zone performance benchmark",
            ...
            with st.expander("How is the benchmark score calculated?", expanded=False):
                st.markdown(
                    "Each zone is scored 0–100 on three equally-weighted pillars, "
                    "then ranked:\n\n"
                    "- **Collection efficiency** — paid ÷ billed (%).\n"
                    "- **Water access** — population safely-managed (%) in the latest year.\n\n"
                    "- **Sanitation access** — population safely-managed (%) in the latest year.\n\n"
                    "The **overall score** is the average of the three. NRW and service "
                    "continuity aren't shown here — those are measured per source, and a "
                    "source can serve several zones, so splitting them by zone would mean "
                    "estimating rather than reporting a real figure. Switch **Country** "
                    "back to **All** for the cross-country view (which does include them)."
                )
        else:
            render_section_header(
            ...
            with st.expander("How is the benchmark score calculated?", expanded=False):
                st.markdown(
                    ...
                    "master users because it exposes cross-country data. Pick a single "
                    "**Country** above to switch this to a cross-zone view instead."
                )
        try:
            from components.benchmarking import render_benchmarking_radar
            render_benchmarking_radar(country=selected_country)
