...
def scene_finance():
    ...
    with st.expander("Data Import", expanded=False):
        ...
        if st.session_state.national_data is not None and st.session_state.fin_service_data is not None:
            ...
            st.warning("No finance data loaded")
    ...
    render_section_header("Daily briefing", eyebrow="Financial overview", icon="today")
    ...
    if avg_collection_rate >= 80:
        coll_kind, coll_label = "positive", "On target"
    elif avg_collection_rate >= 60:
        coll_kind, coll_label = "neutral", "Below target"
    else:
        coll_kind, coll_label = "negative", "Critical"

    # ---- Sparkline series (best-effort) ------------------------------------
    def _fin_series(df, col, agg="sum", n=12):
        if df is None or df.empty or col not in df.columns:
            return []
        try:
            keys = [k for k in ("year", "month") if k in df.columns]
            if keys:
                grouped = df.groupby(keys)[col].agg(agg).reset_index().sort_values(keys)
                vals = grouped[col].tolist()
            else:
                vals = df[col].tolist()
            return [float(v) for v in vals[-n:] if v is not None and not pd.isna(v)]
        except Exception:
            return []

    budget_spark   = _fin_series(national_filtered, "budget_allocated", "sum")
    billed_spark   = _fin_series(fin_service_filtered, "sewer_billed", "sum")
    revenue_spark  = _fin_series(fin_service_filtered, "sewer_revenue", "sum")
    coll_spark     = _fin_series(fin_service_filtered, "collection_rate", "mean")
    ...
    render_section_header("Billing analysis", eyebrow="Revenue performance")
    ...
    render_section_header("Debt & arrears", eyebrow="Outstanding obligations")
    ...
    render_section_header("Financial health indicators", eyebrow="Operational ratios")
    ...
    render_section_header("Data view & export", eyebrow="Underlying datasets")

    export_tab1, export_tab2, export_tab3 = st.tabs(["Financial Service Data", "National Budget Data", "Calculated Metrics"])
