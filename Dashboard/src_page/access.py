...
# `icon` is a Material Symbols name (used in section headers, not in
# Streamlit tab labels — st.tabs renders plain text only).
TAB_STRUCTURE = {
    "Coverage Overview": {
        "icon": "pie_chart",
...
def render_tab_selector(selected_tab: str) -> str:
    ...
    # Native Streamlit tabs render text only — keep labels clean.
...
def render_coverage_overview_tab(df_water, df_sewer, df_service, df_fin, selected_country, selected_zone, selected_year, view_type):
    ...
    render_section_header(
        ...
        eyebrow="Water & sewer",
        icon="pie_chart",
        meta="Service scorecards & access quality",
    )
    
    # Key Performance Scorecards
    render_section_header("Key performance scorecards")
...
def render_growth_metrics_tab(df_water, df_service, selected_country, selected_zone, view_type):
    ...
    render_section_header(
        ...
        eyebrow="Trends",
        icon="trending_up",
        meta="Coverage trends & expansion analysis",
    )
...
def render_infrastructure_tab(df_service, df_fin, selected_country, selected_zone, selected_year, view_type):
    ...
    render_section_header(
        ...
        eyebrow="Assets",
        icon="engineering",
        meta="Metering & facility distribution",
    )
...
def render_equity_demographics_tab(df_water, df_sewer, df_service, selected_country, selected_zone, selected_year, view_type):
    ...
    render_section_header(
        ...
        eyebrow="Population",
        icon="balance",
        meta="Access disparities & equity analysis",
    )
...
def render_transitions_tab(df_water, df_sewer, df_service, selected_country, selected_zone, selected_year, view_type):
    ...
    render_section_header(
        ...
        eyebrow="Movement",
        icon="swap_horiz",
        meta="Population movement & service transitions",
    )
...
def scene_access():
    ...
    with st.expander("Data Import", expanded=False):
        ...
        if st.session_state.access_water_data is not None and st.session_state.access_sewer_data is not None:
            ...
            st.warning("No access data loaded")
    ...
    render_section_header("Key performance scorecards", eyebrow="Snapshot")
    ...
    water_kind = "positive" if muni_yoy_growth >= 0 else "negative"
    sewer_kind = "positive" if sewer_growth >= 0 else "negative"

    render_kpi_row([
        KPI("Water supply coverage", f"{muni_supply_pct:.1f}%",
            delta=f"{muni_yoy_growth:+.1f}% {water_comparison_label}",
    ...
    render_section_header("Ladder analysis", eyebrow="Quality of access", meta="Water · Sanitation")
    ...
    render_section_header("Coverage growth trends", eyebrow="Year on year")
    ...
    render_section_header("Infrastructure metrics", eyebrow="Assets")
    ...
    st.markdown("---")
    render_section_header("Data export", eyebrow="Underlying datasets")
    ...
    st.markdown("---")
    render_section_header("Data quality & alerts", eyebrow="Diagnostics")
